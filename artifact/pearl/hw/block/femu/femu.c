...
//#include "ftl/fde_crypto.h"

#include <immintrin.h>
#include "nvme.h"

// Encoding lookup table for WOM(3,5)
static const uint8_t WOM_encode_map[8] = {
    0b00000, // 000 -> 00000
    0b00001, // 001 -> 00001
    0b00100, // 010 -> 00100
    0b01000, // 011 -> 01000
...
static const uint8_t WOM_decode_map[32] = {
    0b000, // 00000 -> 000
    0b001, // 00001 -> 001
    0b010, // 00100 -> 010
    0b011, // 01000 -> 011
...
void WOM_encode(uint8_t *src, uint8_t *dst, uint32_t len) {
    ...
    for (uint32_t i = 0; i < len; ++i) {
        ...
        for (int bit_group = 0; bit_group < 8; bit_group += 3) {
            uint8_t data_bits = (current_byte >> bit_group) & 0x07; // Extract 3 bits at a time
            temp_data = (temp_data << 5) | WOM_encode_map[data_bits];
            bits_collected += 5;

            if (bits_collected >= 8) {
                dst[dst_index++] = (temp_data >> (bits_collected - 8)) & 0xFF;
                bits_collected -= 8;
            }
        }
    }
    ...
    if (bits_collected > 0) {
        dst[dst_index++] = (temp_data << (8 - bits_collected)) & 0xFF;
    }
}

void WOM_decode(uint8_t *src, uint8_t *dst, uint32_t len) {
    uint32_t dst_index = 0;
    uint8_t temp_data = 0;
    uint8_t bits_collected = 0;
    ...
    for (uint32_t i = 0; i < len; ++i) {
        uint8_t current_byte = src[i];
        temp_data = (temp_data << 8) | current_byte;
        bits_collected += 8;

        while (bits_collected >= 5) {
            uint8_t encoded_bits = (temp_data >> (bits_collected - 5)) & 0x1F; // Extract 5 bits
            uint8_t decoded_bits = WOM_decode_map[encoded_bits];
            if (decoded_bits == 0xFF) {
                // Handle invalid encoding
                // For simplicity, we'll set it to 0 here, but you could handle this differently.
                dst[dst_index++] = 0;
            } else {
                dst[dst_index++] = decoded_bits;
            }
            bits_collected -= 5;
        }
    }
...
static void meta_scan_time(struct ssd *ssd) {
    struct ssdparams *spp = &ssd->sp;
    double total_lat = 0;
    uint64_t start_time = 0, end_time = 0;

    total_lat = 20000 * spp->blks_per_pl * spp->pgs_per_blk; // Read from OOB area. This is a parallel reading from LUNs.
    
    printf("OOB read time: %lf\n", (total_lat * 1.0));
    ...
    for(uint64_t i = 0; i < spp->tt_pgs/100000; i++) {
        hidden_read(ssd, 4096/IV_LEN, ssd->ini_pi, ssd->ini_pi_);
    }
    end_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);

    total_lat += ((end_time - start_time) *100000);

    printf("GMP time: %.lf\n", 1.0*(end_time - start_time) * 100000/1000000000);

    printf("Scanning time is: %.lf\n", 1.0 * total_lat/1000000000);
    ...
    NvmeRequest *req)
{
    ...
    if (n->ssd.pd_mode == 0) {
        if(req->is_write == 0 && n->fde == 1) { //FDE decryption for read requests
            int pg_num = req->nlb/8 + (req->nlb % 8 > 0)?1:0; // pg size is 4KB
  //          assert(req->nlb % 8 == 0);
            // TODO: Recover the original data layout before encryption
            uint64_t start_time, end_time;
            start_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);

            for(int i = 0; i < pg_num; i++) {
                fde_decryption(&(n->ssd), 4096, (n->ssd).crypto_buf, (n->ssd).crypto_buf, (n->ssd).tv_map[req->slba / 8].tv);
                //if(n->ssd.pd_mode == 0)
                    WOM_decode((n->ssd).crypto_buf, (n->ssd).dst_buf, 4096);
                //;
            }
            end_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
            n->ssd.tt_lat_fde += (end_time - start_time);
            n->ssd.num_fde += 1;
       }

        ret = femu_rw_mem_backend_bb(&n->mbe, &req->qsg, data_offset, req->is_write);

        if(req->is_write == 1 && n->fde == 1) { //FDE decryption for write requests
            int pg_num = req->nlb/8 + (req->nlb % 8 > 0)?1:0;
//            assert(req->nlb % 8 == 0);
            uint64_t start_time, end_time;
            start_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);

            for(int i = 0; i < pg_num; i++) {
                fde_encryption(&(n->ssd), 4096, (n->ssd).crypto_buf, (n->ssd).crypto_buf, (n->ssd).tv_map[req->slba / 8].tv);
                if(n->ssd.pd_mode == 0)
                    WOM_encode((n->ssd).crypto_buf, (n->ssd).dst_buf, 4096);

                // Add ranking and unranking here
                //;
            }
            end_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
            n->ssd.tt_lat_fde += (end_time - start_time);
        ...
        if(req->is_write == 0 && n->fde == 1) { //FDE decryption for read requests in hidden mode
            int pg_num = req->nlb/8 + (req->nlb % 8 > 0)?1:0;
    //        assert(req->nlb % 8 == 0);
            uint64_t start_time, end_time;
            start_time = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
        ...
        ret = femu_rw_mem_backend_bb(&n->hidden_mbe, &req->qsg, data_offset, req->is_write);
    ...
    if(req->is_write == 1 && req->slba == SC_OFF && req->nlb == SC_SZ) { // Create a storage channel for PD model management. Data: "model+password". model == 0 is public and model == 1 is hidden.
        char pd_buf[4001];
        char cmd[20];
        char password[20];
        uint32_t read_point;
        uint16_t cmd_len = 20;
        uint16_t passwd_len = 20;

        memcpy(pd_buf, (n->mbe).mem_backend + SC_OFF * SEC_SIZE, 4000);
        
        memcpy(cmd, pd_buf, cmd_len);
        read_point += cmd_len;

        memcpy(password, pd_buf + read_point, passwd_len);

        if(strcmp(cmd, "Public") == 0 && strcmp(password, PUBLIC_PASSWORD) == 0) {
            printf("Public mode!\n");
            n->ssd.pd_mode = 0;
        }
 
        if(strcmp(cmd, "Public+hidden") == 0 && strcmp(password, HIDDEN_PASSWORD) == 0) {
            printf("Public+hidden mode!\n");
            n->ssd.pd_mode = 1;
        }

        if(strcmp(cmd, "Hidden") == 0 && strcmp(password, HIDDEN_PASSWORD) == 0) {
            printf("Hidden mode!\n");
            n->ssd.pd_mode = 2;
        }

        if(strcmp(cmd, "Recover") == 0 && strcmp(password, HIDDEN_PASSWORD) == 0) {
            printf("Recover metadata!\n");
            meta_scan_time(&(n->ssd));
...
static void femu_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    if(n->fde == 1) {
        printf("FDE mode!\n");
        hidden_size = ((int64_t)n->hidden_sz) * 1024 * 1024;

        printf("%"PRIu64"\n", hidden_size);

        femu_init_mem_backend(&n->hidden_mbe, hidden_size);
    }

    n->mbe.femu_mode = n->femu_mode;
    n->hidden_mbe.femu_mode = n->femu_mode;
...
static void femu_exit(PCIDevice *pci_dev)
{
    ...
    femu_destroy_mem_backend(&n->hidden_mbe);
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_UINT32("hidden_mb", FemuCtrl, hidden_sz, 1024), /* Hidden size, Coperd: in MB */
    DEFINE_PROP_UINT8("fde", FemuCtrl, fde, 1), /* FDE enabler. 0 is not enabled, 1 is enabled.*/
