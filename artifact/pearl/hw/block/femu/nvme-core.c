...
static uint16_t nvme_admin_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_ADM_CMD_DELETE_CQ:
        ...
        printf("Page number in the flash: %"PRIu64"\n", n->ssd.pg_in_flash*5/3);
        printf("Total copied page during GC is: %"PRIu64"\n", n->ssd.copied_pg_num);
        printf("WAF value is: %.6lf\n", 1.0 * ((5*n->ssd.pg_in_flash/3) + n->ssd.copied_pg_num)/n->ssd.total_w_pg);

        fprintf(fp, "%.6lf", 1.0 * ((5*n->ssd.pg_in_flash/3) + n->ssd.copied_pg_num)/n->ssd.total_w_pg);
