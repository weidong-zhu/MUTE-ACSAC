...
static uint16_t nvme_admin_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeCqe *cqe)
{
    ...
    case NVME_ADM_CMD_DELETE_CQ:
        ...
        fp = fopen("waf.result", "w");

        printf("Total I/O page: %"PRIu64"\n", n->ssd.total_pg);
        printf("Total write page: %"PRIu64"\n", n->ssd.total_w_pg);
        printf("Total migrated page: %"PRIu64"\n", n->ssd.copied_pg_num);
        printf("Page number in the flash: %"PRIu64"\n", n->ssd.pg_in_flash);
 
        printf("FDE num: %"PRIu64"\n", n->ssd.num_fde);
        printf("FDE lat: %"PRIu64"\n", n->ssd.tt_lat_fde);

        printf("Average FDE latency is: %.6lf us\n", (1.0*n->ssd.tt_lat_fde)/n->ssd.num_fde);
        printf("Average FTL latency is: %.6lf us\n", (1.0*n->ssd.tt_lat_ftl)/n->ssd.num_ftl);
        printf("Average flash latency is: %.6lf us\n", (1.0*n->ssd.tt_flash_lat)/n->ssd.num_ftl);
        printf("Average permutation latency is: %.6lf us\n", (1.0*n->ssd.tt_perm_lat)/n->ssd.num_perm);

        fprintf(fp, "%.6lf", 1.0 * ((n->ssd.pg_in_flash) + n->ssd.copied_pg_num)/n->ssd.total_w_pg);
