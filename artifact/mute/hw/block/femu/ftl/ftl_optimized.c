...
static void *ftl_thread(void *arg)
{
    ...
    while (1) {
        for (int i = 1; i <= n->num_poller; i++) {
            ...
            assert(req);
            //dev_flag = device_selection(ssd, req->is_write);
            switch (req->is_write) {
                case 1:
                  //if(dev_flag == 0)
                    //lat = nvm_write(ssd, req);
                    lat = 2000000;
                  //else {
                    //lat = flash_write(ssd, req);
                  //}
                    break;
                case 0:
                  //if(dev_flag == 0)
                    //lat = nvm_read(ssd, req);
                    lat = 0;
                  //else {
                    //lat = flash_read(ssd, req);
                  //}
#if 0
                if (lat >= 20000000) {
                    lat /= 4;
...
void erase(struct ssd *ssd, struct ppa ppa)
{
    struct ssdparams *spp = &ssd->sp;
    //struct ssd_channel *chp;
    struct nand_lun *lunp;
